...
void CDlgRemoteShowString::OnBnClickedBtnStringOk()
{
    ...
    if (strlen(m_csString) > 44)
    {
        AfxMessageBox("string length over 44");
        return;
    }
    memset(m_struShowString.struStringInfo[i].sString, 0, sizeof(m_struShowString.struStringInfo[i].sString));
    
    string szInfo;
    if (g_struDeviceInfo[m_iDeviceIndex].byCharaterEncodeType == 6) //UTF8
    {
        szInfo = ASCII2UTF8(m_csString);
    }
    else
    {
        szInfo = ASCII2UTF8(m_csString);
    }
    memcpy(m_struShowString.struStringInfo[i].sString, szInfo.c_str(), szInfo.length());
    m_struShowString.struStringInfo[i].wStringSize = (WORD)szInfo.length();
...
void CDlgRemoteShowString::OnBnClickedBtnStringSet()
{
    ...
    memcpy(m_struShowString.struStringInfo[i].sString, szInfo.c_str(), 44);
    
    m_struShowString.struStringInfo[i].wStringSize = (WORD)szInfo.length();//m_csString.GetLength();//strlen(m_csString)
	m_struShowString.struStringInfo[i].wShowStringTopLeftX = (WORD)m_iStringX;
	m_struShowString.struStringInfo[i].wShowStringTopLeftY = (WORD)m_iStringY;	
}
...
void CDlgRemoteShowString::OnCbnSelchangeComboShowArea()
{
    ...
    m_csString.Format("%s", szInfo.c_str());
