...
CDlgITCIOinCfg::CDlgITCIOinCfg(CWnd* pParent /*=NULL*/)
    ...
    , m_ParkingDetectEnabled(FALSE)
    , m_ParkingDetectJudgeTime(0)
{
...
CDlgITCIOinCfg::~CDlgITCIOinCfg()
{
	...
	if (m_lSyncVehHandle >=0)
	{
		...
		{
			g_StringLanType(szLan, "同步名单数据停止失败", "Vehicle Block and Allow List Stop Failed");
			AfxMessageBox(szLan);
			return;
		}
	}
...
void CDlgITCIOinCfg::DoDataExchange(CDataExchange* pDX)
{
    ...
    //}}AFX_DATA_MAP
    DDX_Check(pDX, IDC_CHECK_ENABLE2, m_ParkingDetectEnabled);
    DDX_Text(pDX, IDC_EDIT_PLATE_NUM_VEHICLE2, m_ParkingDetectJudgeTime);
}
...
// 	}
    ...
    m_ParkingDetectEnabled = pstruEntranceTmp->byParkingDetectEnabled;
    m_ParkingDetectJudgeTime = pstruEntranceTmp->byParkingDetectJudgeTime;
	UpdateData(FALSE);
}
...
void CDlgITCIOinCfg::OnBtnDel() 
{
	...
	{
		g_StringLanType(szLan, "删除设备内禁止名单数据库信息失败", "Delete Vehicle Info failed");
...
// 				pThis->GetDlgItem(IDC_EDIT_VEHICLE_INF0_COUNT)->GetWindowText(str);
				g_StringLanType(szLan, "长连接获取名单数据结束!", "Get Block and Allow List Finish");
			...
			{
				g_StringLanType(szLan, "长连接获取名单数据失败", "Get Block and Allow List failed");
				AfxMessageBox(szLan);
				break;
			}
			else
...
void CDlgITCIOinCfg::OnBtnVehicleControl() 
{
	...
	if (m_lHandle >= 0)
	{
		...
		//g_StringLanType(szLan, "请求长连接获取名单数据失败", "Ask Get Block and Allow List failed");
		//AfxMessageBox(szLan);
		//return;
	} 
...
// 		
		...
		{
			g_StringLanType(szLan, "同步名单数据发送失败", "Vehicle Block and Allow List Send Failed");
			sprintf(szLan, "%s 错误码：%d", szLan, NET_DVR_GetLastError());
			AfxMessageBox(szLan);
			return;
		}
...
void CDlgITCIOinCfg::OnBtnCreateLink() 
{
	...
	if (m_lSyncVehHandle < 0)
	{
		g_StringLanType(szLan, "同步名单数据连接失败", "Vehicle Block and Allow List Link Failed");
		sprintf(szLan, "%s 错误码：%d", szLan, NET_DVR_GetLastError());
		AfxMessageBox(szLan);
		return;
	}
...
// 	}
	...
	pstruEntranceTmp->byGateSingleIO[i] = m_comGateSingleIO.GetCurSel();
    pstruEntranceTmp->byParkingDetectEnabled = m_ParkingDetectEnabled;
    pstruEntranceTmp->byParkingDetectJudgeTime = m_ParkingDetectJudgeTime;
