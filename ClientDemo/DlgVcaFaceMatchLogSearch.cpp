...
UINT GetFindFaceMatchLogThread(LPVOID pParam)
{
	...
	while (1) 
	{
        ...
        if (bRet == NET_DVR_FILE_SUCCESS)
		{
			...
			csTmp.Format("%d", struFaceMatchLog.struBlockListInfoLog.struBlockListInfo.dwRegisterID);
			g_pVcaFindMatch->m_listFindData.SetItemText(g_pVcaFindMatch->m_lFaceMatchLogNum, 3, csTmp);

			csTmp.Format("%d", struFaceMatchLog.struBlockListInfoLog.dwBlockListPicID);
		...
		else
		{
			...
			if ((bRet == NET_DVR_NOMOREFILE) || (bRet == NET_DVR_FILE_NOFIND))
			{
				...
				g_StringLanType(szLan, "搜索禁止名单比对报警信息结束!", "Search Facematch Alarm Ending");
				AfxMessageBox(szLan);
				break;
			}
			else
...
void CDlgVcaFaceMatchLogSearch::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_BLOCKLIST_FACE_ID, m_dwBlockListFaceID);
    DDX_Text(pDX, IDC_EDIT_BLOCKLIST_ID, m_dwBlockListID);
...
BOOL CDlgVcaFaceMatchLogSearch::OnInitDialog()
{
	...
	g_StringLanType(szLan, "禁止名单图片ID", "Blocklist Pic ID");
...
void CDlgVcaFaceMatchLogSearch::OnBtnFind() 
{
	...
	if (m_lFindHandle < 0)
	{
		...
		g_StringLanType(szLan, "搜索禁止名单比对报警信息失败", "Search FaceMatch Alarm failed");
		AfxMessageBox(szLan);
        return;
	}
	else
...
void CDlgVcaFaceMatchLogSearch::OnBtnGetFacematchPic() 
{
	...
	struFindCond.dwBlockListID = m_dwBlockListID;
	struFindCond.dwBlockListFaceID = m_dwBlockListFaceID;

	NET_VCA_FACEMATCH_PICTURE struFindRet = {0};
	struFindRet.pSnapFace = new BYTE[1024*1024];
	memset(struFindRet.pSnapFace, 0, 1024*1024);
	struFindRet.pBlockListFace = new BYTE[1024*1024];
    memset(struFindRet.pBlockListFace, 0, 1024*1024);
	...
	if (struFindRet.dwBlockListFaceLen > 0)
	{	
		SYSTEMTIME t;
		GetLocalTime(&t);
		char chTime[128];
		...
		sprintf(cFilename, "%s\\MatchAlarmLog_B[%s][%d].jpg",cFilename, chTime, struFindCond.dwBlockListFaceID);
		hFile = CreateFile(cFilename, GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		if (hFile == INVALID_HANDLE_VALUE)
		{
			return;
		}
		WriteFile(hFile, struFindRet.pBlockListFace, struFindRet.dwBlockListFaceLen, &dwReturn, NULL);
		CloseHandle(hFile);
		hFile = NULL;
	}
}
