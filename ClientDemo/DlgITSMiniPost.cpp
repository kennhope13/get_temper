...
void CDlgITSMiniPost::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHECK_EVENT_BLOCKLIST, m_bChkEvent_BlockList);
...
void CDlgITSMiniPost::AddTrafficDataItem(const NET_DVR_TRAFFIC_DATA_QUERY_RESULT &Item)
{
    ...
    bTemp = (Item.dwVehicleType >> 9) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%sSUV/MPV;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 10) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s中型客车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 11) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s机动车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 12) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s非机动车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 13) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s小型轿车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 14) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s微型轿车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 15) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s皮卡车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 16) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s集装箱卡车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 17) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s微卡/栏板卡;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 18) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s渣土车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 19) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s吊车/工程车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 20) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s油罐车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 21) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s混凝土搅拌车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 22) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s平板拖车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 23) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s两厢轿车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 24) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s三厢轿车;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 25) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s轿跑;", szStr);
    }
    bTemp = (Item.dwVehicleType >> 26) & 0x01;
    if (bTemp)
    {
        sprintf(szStr, "%s小型客车;", szStr);
    }
...
void CDlgITSMiniPost::SetTrafficDataCond()
{
    ...
    m_struTrafficDataQueryCond.dwEventType |= (m_bChkEvent_BlockList << 8);
